...
> **面向长篇小说创作的本地优先 AI 工作台：立项、世界观、人物卡、大纲、章节草稿、审稿、修订、连续性维护与导出。基于 DeepSeek V4 的 100 万 token 上下文和流式推理能力。**
...
# 4. 直接下载 / Docker —— 首个 Writer release 发布后启用。
#    https://github.com/luo-cccc/Writer/releases

# 5. 本地 Docker 构建 —— 不依赖远程镜像。
docker build -t writer .
docker run --rm -it -e DEEPSEEK_API_KEY -v "$PWD:/workspace" writer
```

> 中国大陆访问较慢时，npm 可加 `--registry=https://registry.npmmirror.com`，
> 或使用下方的 [Cargo 镜像](#中国大陆--镜像友好安装)。

[![CI](https://github.com/luo-cccc/Writer/actions/workflows/ci.yml/badge.svg)](https://github.com/luo-cccc/Writer/actions/workflows/ci.yml)
...
![Writer 截图](assets/screenshot.png)

---

## 这是什么？

Writer 是一个完全运行在本地的长篇小说创作工具。它把一本书拆成可维护的工程资产：`book.toml`、故事圣经、世界观、人物卡、章节大纲、草稿、审稿、终稿、事实与伏笔台账、导出文件。

它面向 **DeepSeek V4**（`deepseek-v4-pro` / `deepseek-v4-flash`）构建，原生支持 100 万 token 上下文窗口和思考模式流式输出，重点解决长篇创作里的遗忘、漂移、设定冲突和 AI 腔。

### 主要功能

- **小说项目初始化** —— `deepseek init` 创建本地书籍工程结构
- **总纲生成** —— `deepseek plan` 生成世界观、主角团、卷结构和章节推进表
- **章节简报** —— `deepseek brief N` 生成本章目标、冲突、事实锁、伏笔和追读钩子
- **长篇记忆图** —— `deepseek memory build/context/query/impact` 把人物、地点、规则、章节、伏笔和摘要变成可查询关系图
- **可选写法札记** —— `deepseek empower N` 只提供章节写法可能性，不作为评分表或固定流水线
- **章节创作** —— `deepseek write N` 按项目设定和近期上下文生成章节草稿
- **记忆诊断** —— `deepseek audit N` 检查正文和记忆图的冲突、缺口、影响范围和待确认更新
- **章节修订** —— `deepseek revise N` 根据审稿意见生成终稿
- **章节记忆** —— `deepseek remember N` 抽取摘要、角色状态、事件、事实锁和伏笔台账
- **整书导出** —— `deepseek export` 汇总终稿或草稿到 Markdown/TXT
- **本地优先资产** —— Markdown/TOML/YAML/JSONL，适合手动编辑、备份和版本管理
- **100 万 token 上下文** —— 支持读取大量设定、前文章节和审稿材料
- **三种交互模式** —— Plan（只读规划）、Agent（带审批的默认创作）、YOLO（可信工作区自动执行）
...
- **MCP / 子智能体 / RLM** —— 作为高级能力保留，用于批量审稿、长文本分析和外部工具扩展
...
Writer 可以同时调度多个子智能体并行运行——适合批量审稿、角色状态梳理和伏笔检查：
...
使用 `deepseek --model auto` 或 `/model auto` 让 Writer 自行决定每轮需要多少模型和推理能力。
...
在真实请求发出之前，应用会先用关闭推理的 `deepseek-v4-flash` 进行一次小型路由调用。路由器审视最新请求和最近的上下文，然后为真实请求选定具体的模型和推理强度。简短/简单的轮次保持在 Flash + 关闭推理；总纲设计、长章节生成、连续性审稿、复杂修订或模糊的多步骤创作任务可升级到 Pro 和/或更高推理强度。

`auto` 是 Writer 本地行为。上游 API 永远不会收到 `model: "auto"`，它只会收到为当前轮次选定的具体模型和推理强度设置。界面会显示选定的路由，成本跟踪按实际运行的模型计费。如果路由调用失败或返回无效答案，应用会回退到本地启发式规则。子智能体会继承 auto 模式，除非你为它们指定了显式模型。

需要可重复基准测试、严格控制成本上限或特定提供商/模型映射时，请使用固定模型或固定推理强度。

### Linux ARM64（HarmonyOS 轻薄本、openEuler、Kylin、树莓派、Graviton 等）

从 v0.8.8 起，`npm i -g deepseek-tui` 直接支持 glibc 系的 ARM64 Linux。你也可以从 [Releases 页面](https://github.com/luo-cccc/Writer/releases) 下载预编译二进制，放到 `PATH` 目录中。
...
也可以直接从 [GitHub Releases](https://github.com/luo-cccc/Writer/releases) 下载预编译二进制。`DEEPSEEK_TUI_RELEASE_BASE_URL` 可用于镜像后的 release 资产。

### Windows (Scoop)

[Scoop](https://scoop.sh) 是一个 Windows 软件包管理器。当前公开
Scoop main bucket 中的 `deepseek-tui` manifest 仍指向旧上游发布；在
Writer 首个公开 release 前，优先使用 npm、Cargo 或源码安装：
...
Writer release 发布后再把 Scoop manifest 作为正式安装路径。
...
deepseek init --title 我的长篇 --genre 都市重生 --premise "核心钩子"
deepseek plan --chapters 30              # 生成总纲和章节推进表
deepseek brief 1                         # 生成第 1 章写作简报
deepseek memory build                    # 构建长篇记忆图
deepseek memory context 1                # 查看第 1 章最小必要记忆上下文
deepseek write 1 --words 3500            # 生成第 1 章草稿
deepseek audit 1                         # 诊断第 1 章记忆冲突和待更新项
deepseek revise 1                        # 生成第 1 章终稿
deepseek remember 1                      # 更新章节连续性记忆
deepseek export --format markdown        # 导出整书
...
首个 Writer release 发布后，Docker 镜像会由 release workflow 发布到
GHCR。当前可先本地构建：

```bash
docker build -t writer .
docker run --rm -it -e DEEPSEEK_API_KEY="$DEEPSEEK_API_KEY" writer
```
...
Writer 从工作区目录（`.agents/skills` → `skills` → `.opencode/skills` → `.claude/skills`）和全局 `~/.deepseek/skills` 发现技能。每个技能是一个包含 `SKILL.md` 的目录：
...
- **[OpenWarp](https://github.com/zerx-lab/warp)** — 感谢 OpenWarp 优先支持 Writer，并一起打磨更好的终端智能体体验。
...
欢迎提交 pull request——请先查看 [CONTRIBUTING.md](CONTRIBUTING.md) 并留意[开放 issue](https://github.com/luo-cccc/Writer/issues) 中的好入门任务。
...
[![Star History Chart](https://api.star-history.com/chart?repos=luo-cccc/Writer&type=date&legend=top-left)](https://www.star-history.com/?repos=luo-cccc%2FWriter&type=date&logscale=&legend=top-left)
