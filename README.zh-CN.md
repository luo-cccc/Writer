...
![DeepSeek Novel Studio 截图](assets/screenshot.png)

---

## 这是什么？

DeepSeek Novel Studio 是一个完全运行在本地的长篇小说创作工具。它把一本书拆成可维护的工程资产：`book.toml`、故事圣经、世界观、人物卡、章节大纲、草稿、审稿、终稿、事实与伏笔台账、导出文件。
...
DeepSeek Novel Studio 可以同时调度多个子智能体并行运行——适合批量审稿、角色状态梳理和伏笔检查：
...
使用 `deepseek --model auto` 或 `/model auto` 让 DeepSeek Novel Studio 自行决定每轮需要多少模型和推理能力。
...
`auto` 是 DeepSeek Novel Studio 本地行为。上游 API 永远不会收到 `model: "auto"`，它只会收到为当前轮次选定的具体模型和推理强度设置。界面会显示选定的路由，成本跟踪按实际运行的模型计费。如果路由调用失败或返回无效答案，应用会回退到本地启发式规则。子智能体会继承 auto 模式，除非你为它们指定了显式模型。
