...
enum Commands {
    ...
    /// Create a local long-form novel workspace.
    #[command(after_help = "\
Examples:
...
")]
    ...
    /// Run a non-interactive Novel Studio prompt.
    #[command(after_help = "\
Common forwarded flags:
  --auto                           Enable tool-backed novel-workspace mode
...
Legacy namespace. Prefer the top-level commands:
  ...
  deepseek export
")]
    Novel(TuiPassthroughArgs),
    /// Show novel workspace status and memory counts.
    Status(TuiPassthroughArgs),
...
Examples:
  ...
  deepseek memory candidates --chapter 12
  deepseek memory apply --chapter 12
")]
    Memory(TuiPassthroughArgs),
    /// Plan reproducible long-run experiments without generating chapters.
    #[command(after_help = "\
Examples:
  deepseek experiment plan --name short-run --chapters 10 --workflow targeted_revise
  deepseek experiment plan --name archive-run --chapters 50 --workflow archive,targeted_revise --skill xianxia-craft
  deepseek experiment snapshot --start 1 --end 10 --run-id short-run
")]
    Experiment(TuiPassthroughArgs),
    /// Analyze chapters, characters, timeline, promises, and memory updates with RLM.
    #[command(after_help = "\
Examples:
  deepseek analyze timeline
  deepseek analyze --depth 3 林墨 知识边界
  deepseek analyze 伏笔回收 候选记忆更新
")]
    Analyze(TuiPassthroughArgs),
...
fn run() -> Result<()> {
    ...
    match command {
        ...
        Some(Commands::Novel(args)) => {
            let resolved_runtime = resolve_runtime_for_dispatch(&mut store, &runtime_overrides);
            delegate_to_tui(&cli, &resolved_runtime, tui_args("novel", args))
        }
        Some(Commands::Status(args)) => {
            let resolved_runtime = resolve_runtime_for_dispatch(&mut store, &runtime_overrides);
            delegate_to_tui(&cli, &resolved_runtime, tui_args("status", args))
        }
        Some(Commands::Map(args)) => {
            let resolved_runtime = resolve_runtime_for_dispatch(&mut store, &runtime_overrides);
            delegate_to_tui(&cli, &resolved_runtime, tui_args("map", args))
        }
        Some(Commands::Plan(args)) => {
            let resolved_runtime = resolve_runtime_for_dispatch(&mut store, &runtime_overrides);
            delegate_to_tui(&cli, &resolved_runtime, tui_args("plan", args))
        }
        Some(Commands::Brief(args)) => {
            let resolved_runtime = resolve_runtime_for_dispatch(&mut store, &runtime_overrides);
            delegate_to_tui(&cli, &resolved_runtime, tui_args("brief", args))
        }
        Some(Commands::Empower(args)) => {
            let resolved_runtime = resolve_runtime_for_dispatch(&mut store, &runtime_overrides);
            delegate_to_tui(&cli, &resolved_runtime, tui_args("empower", args))
        }
        Some(Commands::Write(args)) => {
            let resolved_runtime = resolve_runtime_for_dispatch(&mut store, &runtime_overrides);
            delegate_to_tui(&cli, &resolved_runtime, tui_args("write", args))
        }
        Some(Commands::Audit(args)) => {
            let resolved_runtime = resolve_runtime_for_dispatch(&mut store, &runtime_overrides);
            delegate_to_tui(&cli, &resolved_runtime, tui_args("audit", args))
        }
        Some(Commands::Revise(args)) => {
            let resolved_runtime = resolve_runtime_for_dispatch(&mut store, &runtime_overrides);
            delegate_to_tui(&cli, &resolved_runtime, tui_args("revise", args))
        }
        Some(Commands::Diff(args)) => {
            let resolved_runtime = resolve_runtime_for_dispatch(&mut store, &runtime_overrides);
            delegate_to_tui(&cli, &resolved_runtime, tui_args("diff", args))
        }
        Some(Commands::Undo(args)) => {
            let resolved_runtime = resolve_runtime_for_dispatch(&mut store, &runtime_overrides);
            delegate_to_tui(&cli, &resolved_runtime, tui_args("undo", args))
        }
        Some(Commands::Remember(args)) => {
            let resolved_runtime = resolve_runtime_for_dispatch(&mut store, &runtime_overrides);
            delegate_to_tui(&cli, &resolved_runtime, tui_args("remember", args))
        }
        ...
        Some(Commands::Experiment(args)) => {
            let resolved_runtime = resolve_runtime_for_dispatch(&mut store, &runtime_overrides);
            delegate_to_tui(&cli, &resolved_runtime, tui_args("experiment", args))
        }
        Some(Commands::Analyze(args)) => {
            let resolved_runtime = resolve_runtime_for_dispatch(&mut store, &runtime_overrides);
            delegate_to_tui(&cli, &resolved_runtime, tui_args("analyze", args))
        }
...
mod tests {
    ...
    fn parses_global_override_flags() {
        ...
        assert_eq!(cli.workspace, Some(PathBuf::from("/tmp/story")));
    ...
    fn root_help_surface_contains_expected_subcommands_and_globals() {
        ...
        for hidden in ["  novel", "  review", "  apply"] {
            assert!(
                !rendered.contains(hidden),
                "root help should not surface legacy command: {hidden}"
            );
        }
    }
    ...
    fn subcommand_help_surfaces_are_stable() {
        let cases = [
            ...
            (
                "init",
                vec!["deepseek init --title", "--genre", "--premise", "--force"],
            ),
            (
                "memory",
                vec!["build", "context", "query", "impact", "candidates", "apply"],
            ),
            (
                "analyze",
                vec!["timeline", "--depth", "林墨", "伏笔回收", "候选记忆更新"],
            ),
            ("remember", vec!["deepseek remember 12", "--apply"]),
            (
                "map",
                vec!["project", "chapters", "cards", "memory", "--full"],
            ),
            ("diff", vec!["chapter", "draft", "final"]),
            ("undo", vec!["chapter", "saved version"]),
        ];
