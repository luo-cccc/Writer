...
struct Cli {
    ...
    /// YOLO mode: auto-approve trusted novel-project tool use
    #[arg(long)]
    yolo: bool,

    /// Maximum number of concurrent creative sub-agents (1-20)
    #[arg(long)]
...
enum Commands {
    ...
    /// Create a local long-form novel workspace in the current directory
    #[command(after_help = "\
Examples:
  deepseek init --title 我的长篇 --genre 都市重生 --premise 主角带着失败记忆回到十年前
  deepseek init --force
")]
    Init(novel::NovelInitArgs),
    ...
    Exec(ExecArgs),
    /// Legacy compatibility namespace for older scripts
    #[command(hide = true)]
    #[command(after_help = "\
Legacy namespace. Prefer the top-level commands:
  deepseek init
  deepseek plan
  deepseek brief 1
  ...
  deepseek export
")]
    Novel(novel::NovelArgs),
    /// Show novel workspace status and memory counts
    Status,
    /// Show the novel project map: assets, chapters, cards, promises, and memory
    #[command(after_help = "\
Examples:
  deepseek map
  deepseek map --workspace ./my-book
  deepseek map --full
")]
    Map {
    ...
    /// Build optional craft notes for one chapter
    Empower {
        /// Chapter number, starting at 1.
        chapter: u32,
        /// Override model for this operation.
    ...
    /// Draft one chapter from the book memory and outline
    Write {
        /// Chapter number, starting at 1.
        chapter: u32,
        /// Override model for this operation.
    ...
    /// Diagnose continuity and memory risks in a chapter
    Audit {
        /// Chapter number, starting at 1.
        chapter: u32,
        /// Override model for this operation.
    ...
    /// Show chapter draft/final or saved-version differences
    Diff {
        /// Chapter number, starting at 1.
        chapter: u32,
    },
    /// Restore the latest saved version for one chapter
    Undo {
        /// Chapter number, starting at 1.
        chapter: u32,
    },
    /// Extract chapter memory into reviewable update candidates
    Remember {
        /// Chapter number, starting at 1.
        chapter: u32,
        /// Override model for this operation.
    ...
    Memory {
        #[command(subcommand)]
        command: novel::MemoryCommand,
    },
    /// Plan reproducible long-run experiments without generating chapters
    Experiment {
        #[command(subcommand)]
        command: novel::NovelExperimentCommand,
    },
    /// Analyze chapters, characters, timeline, promises, and memory updates with RLM
    Analyze {
        /// Manuscript-analysis focus, e.g. timeline, 林墨, 伏笔回收.
        #[arg(value_name = "FOCUS", trailing_var_arg = true)]
        focus: Vec<String>,
        /// Maximum recursive RLM depth for semantic slices.
        #[arg(long, default_value_t = 2)]
        depth: u32,
    },
    /// Export finalized chapters, falling back to drafts
    Export {
        /// Output file path. Defaults to exports/<title>.md or .txt.
        #[arg(long)]
        output: Option<PathBuf>,
        /// Export format.
        #[arg(long, value_enum, default_value_t = novel::ExportFormat::Markdown)]
        format: novel::ExportFormat,
    },
    /// Legacy compatibility: diagnose a git diff with Novel Studio
    #[command(hide = true)]
    Review(ReviewArgs),
    /// Legacy compatibility: open the TUI pre-seeded with a GitHub PR's title, body, and diff
    #[command(hide = true)]
    Pr {
        /// PR number
    ...
    /// Legacy compatibility: apply a patch file (or stdin) to the working tree
    #[command(hide = true)]
    Apply(ApplyArgs),
...
async fn main() -> Result<()> {
    ...
    if let Some(command) = cli.command.clone() {
        return match command {
            ...
            Commands::Init(args) => init_project(&cli, args),
            ...
            Commands::Novel(args) => {
                let config = load_config_from_cli(&cli)?;
                let workspace = resolve_workspace(&cli);
                novel::run(&config, args, workspace).await
            }
            Commands::Status => {
                let config = load_config_from_cli(&cli)?;
                let workspace = resolve_workspace(&cli);
                novel::run(
                    &config,
                    novel::NovelArgs {
                        command: novel::NovelCommand::Status,
                    },
                    workspace,
                )
                .await
            ...
            Commands::Map { full } => {
                ...
                novel::run(
                    &config,
                    novel::NovelArgs {
                        command: novel::NovelCommand::Map { full },
                    },
                    workspace,
                )
                .await
            ...
            Commands::Plan {
                ...
                novel::run(
                    &config,
                    novel::NovelArgs {
                        command: novel::NovelCommand::Plan {
                            model,
                            brief,
                            chapters,
                        },
            ...
            Commands::Brief {
                ...
                novel::run(
                    &config,
                    novel::NovelArgs {
                        command: novel::NovelCommand::Brief {
                            chapter,
                            model,
                            direction,
                            force,
            ...
            Commands::Empower {
                ...
                novel::run(
                    &config,
                    novel::NovelArgs {
                        command: novel::NovelCommand::Empower {
                            chapter,
                            model,
                            direction,
                            force,
            ...
            Commands::Write {
                ...
                novel::run(
                    &config,
                    novel::NovelArgs {
                        command: novel::NovelCommand::Write {
                            chapter,
                            model,
                            words,
                            direction,
            ...
            Commands::Audit {
                ...
                novel::run(
                    &config,
                    novel::NovelArgs {
                        command: novel::NovelCommand::Audit {
                            chapter,
                            model,
                            force,
                        },
            ...
            Commands::Revise {
                ...
                novel::run(
                    &config,
                    novel::NovelArgs {
                        command: novel::NovelCommand::Revise {
                            chapter,
                            model,
                            direction,
                            force,
            ...
            Commands::Diff { chapter } => {
                let config = load_config_from_cli(&cli)?;
                let workspace = resolve_workspace(&cli);
                novel::run(
                    &config,
                    novel::NovelArgs {
                        command: novel::NovelCommand::Diff { chapter },
                    },
                    workspace,
                )
                .await
            }
            Commands::Undo { chapter } => {
                let config = load_config_from_cli(&cli)?;
                let workspace = resolve_workspace(&cli);
                novel::run(
                    &config,
                    novel::NovelArgs {
                        command: novel::NovelCommand::Undo { chapter },
                    },
                    workspace,
                )
                .await
            ...
            Commands::Remember {
                ...
                novel::run(
                    &config,
                    novel::NovelArgs {
                        command: novel::NovelCommand::Remember {
                            chapter,
                            model,
                            force,
                            apply,
            ...
            Commands::Memory { command } => {
                ...
                novel::run(
                    &config,
                    novel::NovelArgs {
                        command: novel::NovelCommand::Memory { command },
                    },
                    workspace,
                )
                .await
            ...
            Commands::Experiment { command } => {
                ...
                novel::run(
                    &config,
                    novel::NovelArgs {
                        command: novel::NovelCommand::Experiment { command },
                    },
                    workspace,
                )
                .await
            }
            Commands::Analyze { focus, depth } => {
                let config = load_config_from_cli(&cli)?;
                let workspace = resolve_workspace(&cli);
                let focus = if focus.is_empty() {
                    "全书连续性、人物状态、时间线、伏笔、知识边界和候选记忆更新。".to_string()
                } else {
                    focus.join(" ")
                };
                let message = build_cli_manuscript_analysis_message(&workspace, &focus, depth)?;
                let model = config
                    .default_text_model
                    .clone()
                    .unwrap_or_else(|| config.default_model());
            ...
            Commands::Export { output, format } => {
                ...
                novel::run(
                    &config,
                    novel::NovelArgs {
                        command: novel::NovelCommand::Export { output, format },
                    },
                    workspace,
                )
                .await
...
1. Find the book root (`book.toml`) before reading broadly.\n\
2. Treat `bible/`, `cards/`, `outline/`, chapters, and `memory/` as canon; external material is reference only.\n\
3. For planning or drafting, route output into `outline/`, `chapters/NNN/brief.md`, `chapters/NNN/craft_plan.md`, or `chapters/NNN/draft.md` as appropriate.\n\
4. For diagnosis, write findings into `chapters/NNN/audit.md` with continuity risks and candidate memory updates; do not score prose or force a template.\n\
5. Preserve user prose. Before revising `final.md`, make or rely on a `.versions/` backup and report the exact changed files.\n\
"
    )
}
...
/// Initialize a new long-form novel workspace.
fn init_project(cli: &Cli, args: novel::NovelInitArgs) -> Result<()> {
    use crate::palette;
    use colored::Colorize;
    ...
    if manifest_path.exists() && !args.force {
        println!(
            "{} book.toml already exists at {}",
            "!".truecolor(sky_r, sky_g, sky_b),
            manifest_path.display()
        );
        println!("Use `deepseek init --force` to refresh templates.");
        return Ok(());
    }
    ...
    match novel::initialize_project(
        ...
        Ok(outcome) => {
            ...
            println!();
            println!("Novel workspace: {}", outcome.root.display());
        ...
        Err(e) => {
            println!(
                "{} Failed to create novel workspace: {}",
                "✗".truecolor(red_r, red_g, red_b),
                e
            );
        }
...
fn build_cli_manuscript_analysis_message(
    workspace: &Path,
    focus: &str,
    depth: u32,
) -> Result<String> {
    let prompt = commands::build_novel_analysis_prompt(workspace, focus);
    let source_packet = novel::manuscript_analysis_index_packet(workspace, focus)?;
    let source_arg = format!("content: {source_packet:?}");
    Ok(format!(
        "Use the persistent RLM tools for this manuscript analysis. Open `rlm_open` with name `novel_analysis` and {source_arg}. Configure `sub_rlm_max_depth: {depth}`. Then evaluate this task through bounded helpers and finalize with coverage, findings, and candidate memory updates:\n\n{prompt}"
    ))
}
...
async fn run_review(config: &Config, args: ReviewArgs) -> Result<()> {
    ...
    let system = SystemPrompt::Text(
        "You are a long-form fiction memory diagnostician. This legacy diff path exists for compatibility; focus on changes that could affect novel assets, continuity, memory ledgers, chapter order, or reader-promise risk. \
Provide findings ordered by story risk with file references, then open questions, then a brief summary. Do not force creative prose into a template."
            .to_string(),
    );
    let user_prompt = format!(
        "Diagnose the following diff for novel continuity and memory implications:\n\n{diff}\n\nEnd of diff."
    );
...
mod terminal_mode_tests {
    ...
    #[test]
    fn top_level_map_accepts_full_flag() {
        let cli = parse_cli(&["deepseek", "map", "--full"]);
        let Some(Commands::Map { full }) = cli.command else {
    ...
    #[test]
    fn analyze_command_accepts_depth_and_focus_words() {
        let cli = parse_cli(&["deepseek", "analyze", "--depth", "3", "林墨", "知识边界"]);
        let Some(Commands::Analyze { focus, depth }) = cli.command else {
            panic!("expected analyze command");
        };

        assert_eq!(depth, 3);
        assert_eq!(focus, vec!["林墨", "知识边界"]);
    }

    #[test]
    fn remember_command_accepts_explicit_apply_confirmation() {
        let cli = parse_cli(&["deepseek", "remember", "12", "--apply"]);
        let Some(Commands::Remember { chapter, apply, .. }) = cli.command else {
    ...
    #[test]
    fn cli_analysis_message_uses_inline_rlm_index() {
        let tmp = tempfile::TempDir::new().expect("temp dir");
        novel::initialize_project(
            tmp.path(),
            novel::NovelInitOptions {
                title: Some("CLI 分析测试".to_string()),
        ...
        .expect("init project");
        std::fs::create_dir_all(tmp.path().join("chapters/001")).expect("chapter dir");
        std::fs::write(
        ...
        let message =
            build_cli_manuscript_analysis_message(tmp.path(), "林墨 知识边界", 3).expect("message");
...
mod setup_helper_tests {
    ...
    fn setup_skills_template_is_novel_studio_oriented() {
        ...
        assert!(body.contains("chapters/NNN/audit.md"));
        assert!(body.contains(".versions/"));
        assert!(!body.contains("Quick repo diagnostics"));
        assert!(!body.contains("Cargo.toml"));
        assert!(!body.contains("git_diff"));
    }
}
