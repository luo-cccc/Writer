...
mod stream_decoder_tests {
    ...
    struct TestSpilloverRoot {
        prior: Option<std::path::PathBuf>,
    }

    impl Drop for TestSpilloverRoot {
        fn drop(&mut self) {
            crate::tools::truncate::set_test_spillover_root(self.prior.take());
        }
    }

    fn isolate_spillover(tmp: &tempfile::TempDir) -> TestSpilloverRoot {
        let prior =
            crate::tools::truncate::set_test_spillover_root(Some(tmp.path().join("tool_outputs")));
        TestSpilloverRoot { prior }
    }
    ...
    fn request_builder_truncates_large_tool_result_for_wire() {
        ...
        let tmp = tempfile::TempDir::new().expect("temp spillover root");
        let _spillover_root = isolate_spillover(&tmp);
