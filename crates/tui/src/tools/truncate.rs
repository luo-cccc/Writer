...
pub fn write_sha_spillover(sha: &str, content: &str) -> io::Result<PathBuf> {
    ...
    if let Err(err) = crate::utils::write_atomic(&path, content.as_bytes()) {
        // Another test or turn may have raced us and written the same
        // content-addressed file after the existence check. Treat that as
        // success only when the existing bytes match the expected content.
        if path.exists()
            && fs::read(&path)
