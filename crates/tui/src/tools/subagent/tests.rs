...
fn test_agent_type_verifier_aliases() {
    // Verifier accepts test/validate aliases distinct from the memory
    // diagnostic role, which reads book assets and reports continuity risk.
...
fn test_implementer_and_verifier_have_distinct_prompts() {
    ...
    assert!(
        implementer.to_lowercase().contains("implement")
            || implementer.to_lowercase().contains("drafting")
            || implementer.to_lowercase().contains("revision"),
        "Implementer prompt should reference its role: {implementer}"
    );
    assert!(
        verifier.to_lowercase().contains("verif")
            || verifier.to_lowercase().contains("book gates")
...
fn test_agent_type_prompts_include_shared_output_contract_once() {
    for (agent_type, marker) in [
        (SubAgentType::General, "long-form fiction sub-agent"),
        (SubAgentType::Explore, "novel exploration sub-agent"),
        (SubAgentType::Plan, "story planning sub-agent"),
        (
            SubAgentType::Review,
            "long-form memory diagnostic sub-agent",
        ),
        (SubAgentType::Implementer, "drafting/revision sub-agent"),
        (SubAgentType::Verifier, "continuity verification sub-agent"),
        ...
        assert!(prompt.contains("reader-promise"));
        assert!(prompt.contains("external-material boundary"));
...
fn explore_prompt_orients_before_searching() {
    ...
    assert!(prompt.contains("book root (`book.toml`)"));
    assert!(prompt.contains("memory graph"));
    assert!(prompt.contains("compressed story reconnaissance"));
}
...
fn agent_open_description_explains_fresh_vs_forked_context_and_trust_model() {
    ...
    assert!(description.contains("fresh child with an independent book-task prefill"));
    ...
    assert!(description.contains("chapter edits"));
    assert!(description.contains("memory-ledger writes"));
    assert!(
...
#[test]
fn agent_spawn_description_prioritizes_book_asset_verification() {
    let tmp = tempdir().expect("tempdir");
    let manager = new_shared_subagent_manager(tmp.path().to_path_buf(), 1);
    let tool = AgentSpawnTool::new(manager, stub_runtime());
    let description = tool.description();

    assert!(description.contains("Book side effect"));
    assert!(description.contains("Chapter, card, outline, or bible file"));
    assert!(description.contains("memory/candidates/*.json"));
    assert!(description.contains("exports/"));
    assert!(!description.contains("Git operation"));
    assert!(!description.contains("Test claimed passing"));
}
...
fn subagent_auto_model_routes_unconfigured_assignments() {
    ...
    assert_eq!(
        fallback_subagent_assignment_route(&runtime, None, "audit chapter 12 continuity").model,
        "deepseek-v4-pro"
    );
    assert_eq!(
