...
pub enum SubAgentType {
    ...
    #[default]
    General,
    /// Fast exploration - read-only tools for novel assets.
    Explore,
    /// Planning - analysis tools for story architecture.
    Plan,
    /// Continuity and long-form memory diagnosis - read + analysis tools.
    Review,
    /// Drafting/revision — focused on landing a specific book artifact.
    Implementer,
    /// Continuity verification — focused on reporting pass/fail with evidence.
    Verifier,
...
impl SubAgentType {
    ...
    pub fn from_str(s: &str) -> Option<Self> {
        match s.to_lowercase().as_str() {
            ...
            "review" | "memory-review" | "continuity" | "continuity-review"
            | "memory_diagnostic" | "memory-diagnostic" | "reviewer" => Some(Self::Review),
...
impl ToolSpec for AgentOpenTool {
    ...
    fn description(&self) -> &'static str {
        concat!(
            "Open a named child writing sub-agent session for focused Novel Studio work. Returns the session name, status, agent_id, context_mode, prefix_cache metadata, and a handle_read-compatible transcript_handle. ",
            "Use agent_eval to fetch or wait on the session, and agent_close to cancel/close it.\n\n",
            "Context control is explicit: omit fork_context or set it false for a fresh child with an independent book-task prefill; set fork_context=true for perspective fanout over the current parent context. ",
            ...
            "Sub-agent results are self-reports. Re-verify claimed book side effects such as chapter edits, memory-ledger writes, exports, command results, or external resource writes before reporting them as facts."
        )
    }
    ...
    fn input_schema(&self) -> Value {
        json!({
            ...
            "properties": {
                ...
                "type": {
                    "type": "string",
                    "description": "Sub-agent type: general, explore, plan, review, implementer, verifier, custom. In Novel Studio, these are writing roles: reader/researcher, story planner, memory diagnostician, drafter/reviser, and continuity verifier."
                },
                "agent_type": {
                    "type": "string",
                ...
                "role": {
                    "type": "string",
                    "description": "Role alias for book work: worker, explorer, awaiter, default"
                },
                "agent_role": {
                    "type": "string",
                ...
                "cwd": {
                    "type": "string",
                    "description": "Optional working directory for the child; must be inside the parent book workspace"
                },
                "resident_file": {
                    "type": "string",
...
impl ToolSpec for AgentSpawnTool {
    ...
    fn description(&self) -> &'static str {
        concat!(
            "Spawn a background writing sub-agent for a focused Novel Studio task. Returns an agent_id immediately; follow with agent_result to retrieve the final result. Default cap of 10 concurrent sub-agents (configurable via `[subagents].max_concurrent`); each is a full sub-agent loop, so cancel or wait if you hit the cap. For parallel one-shot LLM queries, just emit multiple tool calls in one turn — the dispatcher runs them in parallel.\n\n",
            "## Trust model: subagent results are self-reports, not verified facts\n\n",
            "`agent_result` returns the child's narrative summary of what happened. For operations with durable book side effects, the child's summary may be wrong. Re-verify before reporting success to the user:\n\n",
            "| Book side effect | Re-verify with |\n|---|---|\n| Chapter, card, outline, or bible file claimed created | `read_file` or `list_dir` |\n| Chapter text claimed edited | `read_file`, then compare against the requested scene or continuity target |\n| Memory candidate or ledger claimed written | `read_file` for `memory/candidates/*.json` or `memory/*.jsonl` |\n| Export claimed produced | `list_dir` / `read_file` under `exports/` |\n| Command claimed passed | inspect the command exit code and output |\n| External URL/resource claimed updated | fetch or inspect the resource directly |\n\n",
            "If the child returns a verifiable handle (file path, command exit code, URL, export path, or memory candidate path), check it. If it doesn't, ask the child to return one or verify yourself before proceeding."
        )
    }
    ...
    fn input_schema(&self) -> Value {
        json!({
            ...
            "properties": {
                ...
                "cwd": {
                    "type": "string",
                    "description": "Optional working directory for the child. Must be inside the parent's book workspace (use a relative path or an absolute path under the workspace root). Use this to isolate large book tasks in a prepared subdirectory when needed."
                },
                "resident_file": {
                    "type": "string",
...
You are the DeepSeek Novel Studio sub-agent routing manager. Return only compact JSON: \
...
Use Pro for long-form story planning, chapter drafting, continuity/memory diagnosis, multi-file \
book changes, high-risk canon decisions, ambiguous requests, or work likely to need tool-call judgment. Use thinking \
...
#[cfg(test)]
mod novel_studio_agent_type_tests {
    use super::*;

    #[test]
    fn review_agent_accepts_memory_diagnostic_aliases_not_code_review() {
        assert_eq!(
            SubAgentType::from_str("memory-diagnostic"),
            Some(SubAgentType::Review)
        );
        assert_eq!(
            SubAgentType::from_str("continuity"),
            Some(SubAgentType::Review)
        );
        assert_eq!(SubAgentType::from_str("code-review"), None);
        assert_eq!(SubAgentType::from_str("code_review"), None);
    }
}
...
const GENERAL_AGENT_INTRO: &str = concat!(
    "You are a long-form fiction sub-agent spawned to handle a specific book task autonomously.\n",
    "Stay inside the assigned novel scope; put adjacent story risks under RISKS/BLOCKERS.\n",
    "For multi-step writing work, use `checklist_write`; add `update_plan` only for complex story strategy.\n\n"
);

const EXPLORE_AGENT_INTRO: &str = concat!(
    "You are a novel exploration sub-agent (role: `explore`). Map relevant book assets quickly and stay read-only.\n",
    "Orient first: confirm the book root (`book.toml`), then read only the relevant bible files, cards, outline, chapters, summaries, and memory graph.\n",
    "Use list_dir/file_search, grep_files, and read_file; use RLM only for long manuscripts or many semantic slices, not basic path discovery.\n",
    "DeepSeek V4 can hold broad evidence, but your value is compressed story reconnaissance: cite `path:line-range` for each finding and stop once evidence is sufficient.\n",
    "CHANGES will almost always be \"None.\" for an explorer.\n\n"
);

const PLAN_AGENT_INTRO: &str = concat!(
    "You are a story planning sub-agent. Produce a grounded, prioritized creative plan, not code patches.\n",
    "Read enough bible/cards/outline/chapter memory to avoid guessing; each step names its book artifact and verification.\n",
    "Use update_plan/checklist_write for plan artifacts and explain story trade-offs without turning creativity into a fixed template.\n",
    "CHANGES should list plan artifacts only, not future speculative edits.\n\n"
);

const REVIEW_AGENT_INTRO: &str = concat!(
    "You are a long-form memory diagnostic sub-agent. Stay read-only and report continuity findings.\n",
    "Read the affected chapters/assets and the memory graph; order EVIDENCE by story risk, not by a style score.\n",
    "Use BLOCKER/MAJOR/MINOR/NIT for continuity conflicts, knowledge-boundary leaks, timeline drift, broken promises, and canon gaps; include path:line-range plus a compatible fix.\n",
    "Do not force a chapter into a formula. If no MAJOR+ memory risks exist, say so plainly in SUMMARY.\n",
    "CHANGES will almost always be \"None.\" for a memory diagnostician.\n\n"
);
...
const IMPLEMENTER_AGENT_INTRO: &str = concat!(
    "You are a drafting/revision sub-agent. Land the assigned novel artifact with focused surrounding edits.\n",
    "Read target bible/cards/outline/chapter/memory files before editing; preserve user prose unless the assignment explicitly asks for replacement.\n",
    "After edit batches, verify the artifact exists and that continuity/memory references still line up.\n",
    "CHANGES is load-bearing: list every modified file with a one-line why.\n\n"
);

const VERIFIER_AGENT_INTRO: &str = concat!(
    "You are a continuity verification sub-agent. Check requested book gates and stay read-only unless explicitly assigned otherwise.\n",
    "Report PASS/FAIL/FLAKY at the top of SUMMARY with exact file or command evidence.\n",
    "Capture contradictions, missing memory updates, broken chapter order, and file:line evidence; put obvious fixes under RISKS.\n",
