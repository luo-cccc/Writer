...
fn build_empty_state_lines(app: &App, area: Rect) -> Vec<Line<'static>> {
    ...
    let body = vec![
        ...
        Line::from(""),
        Line::from(Span::styled(
            format!("{inset}book: {}", novel_workspace_label(&app.workspace)),
            Style::default().fg(palette::TEXT_PRIMARY),
        )),
        Line::from(Span::styled(
            format!("{inset}memory: book.toml + bible/cards/chapters/graph"),
            Style::default().fg(palette::TEXT_MUTED),
        )),
        Line::from(Span::styled(
        ...
        Line::from(""),
        Line::from(Span::styled(
            format!("{inset}Start: /init, /home, or ask for the next chapter."),
            Style::default().fg(palette::TEXT_MUTED),
        )),
    ];
...
fn novel_workspace_label(workspace: &std::path::Path) -> String {
...
#[cfg(test)]
const COMPOSER_PLACEHOLDER: &str = "Ask for a chapter, memory check, or use /.";
...
mod tests {
    ...
    #[test]
    fn slash_completion_hints_exclude_legacy_review_paths() {
        let hints = slash_completion_hints("/", 128, &[], Locale::En, None);
        assert!(hints.iter().any(|hint| hint.name == "/audit"));
        assert!(!hints.iter().any(|hint| hint.name == "/review"));
        assert!(!hints.iter().any(|hint| hint.name == "/apply"));
    }
    ...
    fn empty_composer_cursor_matches_placeholder_padding() {
        ...
        // input_rows_budget = 3
        // placeholder_visual_lines(38) = 2  (current placeholder wraps at 38)
        // top_padding = 3 - clamp(2, 1, 3) = 1
        // cursor_x = 0 + (1-0) + 0 = 1
        // cursor_y = 0 + (1-0) + (1+0) = 2
        assert_eq!(placeholder_visual_lines(38), 2);
    ...
    fn empty_composer_cursor_accounts_for_placeholder_wrapping() {
        ...
        // input_rows_budget = 3
        // placeholder_visual_lines(12) = 4  (clamped to the 3-row input budget)
        // top_padding = 3 - clamp(4, 1, 3) = 0
        // cursor_x = 0 + (1-0) + 0 = 1
        // cursor_y = 0 + (1-0) + (0+0) = 1
        assert_eq!(placeholder_visual_lines(12), 4);
    ...
    fn empty_composer_cursor_uses_full_area_when_border_disabled() {
        ...
        assert_eq!(placeholder_visual_lines(40), 2);
