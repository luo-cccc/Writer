//! Project context loading for Writer.
//!
...
#[derive(Debug, Serialize)]
struct NovelContextPack {
    manifest: Option<String>,
    bible_files: Vec<NovelAssetPack>,
    craft_files: Vec<NovelAssetPack>,
    character_cards: Vec<NovelAssetPack>,
    world_cards: Vec<NovelAssetPack>,
    location_cards: Vec<NovelAssetPack>,
    outline_files: Vec<NovelAssetPack>,
    materials: Vec<NovelAssetPack>,
    material_policy: &'static str,
    memory_schema: Option<NovelAssetPack>,
    recent_summaries: Vec<NovelAssetPack>,
    memory_reports: Vec<NovelAssetPack>,
    memory_candidates: Vec<NovelAssetPack>,
    chapter_dirs: Vec<String>,
    memory_graph: Option<MemoryGraphPack>,
}

#[derive(Debug, Serialize)]
struct NovelAssetPack {
    path: String,
    excerpt: String,
}

#[derive(Debug, Serialize)]
struct MemoryGraphPack {
    path: String,
...
pub fn generate_project_context_pack(workspace: &Path) -> Option<String> {
    ...
    let pack = ProjectContextPack {
        project_name: workspace
            ...
            .unwrap_or("workspace")
            .to_string(),
        novel: generate_novel_context_pack(workspace, &entries),
...
fn generate_novel_context_pack(workspace: &Path, entries: &[String]) -> Option<NovelContextPack> {
    if !workspace.join("book.toml").is_file() {
        return None;
    }

    let manifest = read_pack_excerpt(workspace, "book.toml", 2_000);
    let bible_files = collect_novel_assets(workspace, entries, "bible/", 4_000, 12);
    let craft_files = collect_novel_assets(workspace, entries, "craft/", 3_000, 8);
    let character_cards = collect_novel_assets(workspace, entries, "cards/characters/", 2_500, 32)
        .into_iter()
        .filter(|asset| !asset.path.ends_with("_template.yaml"))
        .collect();
    let world_cards = collect_novel_assets(workspace, entries, "cards/world/", 2_500, 32);
    let location_cards = collect_novel_assets(workspace, entries, "cards/locations/", 2_500, 32);
    let outline_files = collect_novel_assets(workspace, entries, "outline/", 5_000, 12);
    let materials = collect_novel_assets(workspace, entries, "materials/", 2_500, 16);
    let memory_schema =
        read_pack_excerpt(workspace, "memory/SCHEMA.md", 4_000).map(|excerpt| NovelAssetPack {
            path: "memory/SCHEMA.md".to_string(),
            excerpt,
        });
    let mut recent_summaries =
        collect_novel_assets(workspace, entries, "memory/summaries/", 3_000, 12);
    recent_summaries.sort_by(|a, b| b.path.cmp(&a.path));
    recent_summaries.truncate(8);
    let mut memory_reports = collect_novel_assets(workspace, entries, "memory/reports/", 4_000, 12);
    memory_reports.sort_by(|a, b| b.path.cmp(&a.path));
    memory_reports.truncate(4);
    let mut memory_candidates =
        collect_novel_assets(workspace, entries, "memory/candidates/", 3_000, 12);
    memory_candidates.sort_by(|a, b| b.path.cmp(&a.path));
    memory_candidates.truncate(8);

    let mut chapter_dirs = entries
        .iter()
        .filter(|path| is_chapter_dir(path))
        .cloned()
        .collect::<Vec<_>>();
    chapter_dirs.sort();
    if chapter_dirs.len() > 24 {
        chapter_dirs = chapter_dirs.into_iter().rev().take(24).collect::<Vec<_>>();
        chapter_dirs.reverse();
    }

    Some(NovelContextPack {
        manifest,
        bible_files,
        craft_files,
        character_cards,
        ...
        outline_files,
        materials,
        material_policy: "Local materials are reference sources only. They are not canon and must not override book.toml, bible/, cards/, outline/, chapters, or memory/; promote durable facts into canon assets with source notes first.",
        memory_schema,
        recent_summaries,
        memory_reports,
        memory_candidates,
        chapter_dirs,
        memory_graph: read_memory_graph_pack(workspace),
    })
}

fn collect_novel_assets(
    workspace: &Path,
    entries: &[String],
    prefix: &str,
    max_chars: usize,
    ...
    let mut assets = entries
        ...
        .filter_map(|path| {
            read_pack_excerpt(workspace, path, max_chars).map(|excerpt| NovelAssetPack {
                path: path.clone(),
                excerpt,
            })
        })
...
fn read_pack_excerpt(workspace: &Path, path: &str, max_chars: usize) -> Option<String> {
...
fn is_chapter_dir(path: &str) -> bool {
    let Some(rest) = path.strip_prefix("chapters/") else {
        return false;
    };
    let chapter = rest.trim_end_matches('/');
    !chapter.is_empty() && chapter.chars().all(|ch| ch.is_ascii_digit())
}

fn read_memory_graph_pack(workspace: &Path) -> Option<MemoryGraphPack> {
    let path = "memory/graph.json";
    let raw = fs::read_to_string(workspace.join(path)).ok()?;
    ...
    for node in &nodes {
        if let Some(kind) = node.get("kind").and_then(serde_json::Value::as_str) {
...
fn auto_generate_context(workspace: &Path) -> Option<String> {
    ...
    let content = format!(
        "# Project Structure (Auto-generated)\n\n\
         > This file was automatically generated by Writer.\n\
...
This file provides guidance to AI agents (Writer, Claude Code, etc.) when working with code in this repository.
...
mod tests {
    ...
    fn project_context_pack_is_stable_and_sorted() {
        ...
        assert!(first.contains("\"novel\": null"));
    ...
    #[test]
    fn project_context_pack_promotes_novel_memory_assets() {
        let tmp = tempdir().expect("tempdir");
        fs::write(
        ...
        fs::create_dir_all(tmp.path().join("bible")).expect("mkdir bible");
        fs::write(
            tmp.path().join("bible").join("premise.md"),
            "主角找回失踪的姐姐。",
        )
        .expect("write premise");
        fs::create_dir_all(tmp.path().join("cards").join("characters")).expect("mkdir characters");
        fs::write(
            tmp.path().join("cards").join("characters").join("lin.yaml"),
            "name: 林墨\nstate: 不信任任何人\n",
        )
        .expect("write character");
        fs::create_dir_all(tmp.path().join("materials").join("sources")).expect("mkdir materials");
        fs::write(
            tmp.path()
                .join("materials")
                .join("sources")
                .join("missing_case.md"),
            "真实案件资料只能作为参考来源。",
        )
        .expect("write material");
        fs::create_dir_all(tmp.path().join("memory").join("summaries")).expect("mkdir summaries");
        fs::write(
            tmp.path().join("memory").join("SCHEMA.md"),
            "# Memory Graph Schema\n\nschema_version 2\n",
        )
        .expect("write schema");
        fs::create_dir_all(tmp.path().join("memory").join("reports")).expect("mkdir reports");
        fs::write(
            tmp.path()
                .join("memory")
                .join("reports")
                .join("20260514T000000Z-analysis.md"),
            "# Imported RLM Manuscript Analysis\n\n- source: `analysis.md`\n\n## Finding\n时间线风险。",
        )
        .expect("write report");
        fs::create_dir_all(tmp.path().join("memory").join("candidates")).expect("mkdir candidates");
        fs::write(
            tmp.path()
                .join("memory")
                .join("candidates")
                .join("001.json"),
            r#"{"chapter":1,"candidates":[{"kind":"knowledge","target":"林墨","change":"知道姐姐留下暗号"}]}"#,
        )
        .expect("write candidates");
        fs::write(
            tmp.path().join("memory").join("graph.json"),
            r#"{"schema_version":2,"updated_at":"now","nodes":[{"kind":"character"}],"edges":[{}],"candidate_updates":[{}]}"#,
        )
        .expect("write graph");
        fs::create_dir_all(tmp.path().join("chapters").join("001")).expect("mkdir chapter");

        let pack = generate_project_context_pack(tmp.path()).expect("pack");

        assert!(pack.contains("\"novel\""));
        assert!(pack.contains("\"manifest\""));
        assert!(pack.contains("记忆测试"));
        assert!(pack.contains("cards/characters/lin.yaml"));
        assert!(pack.contains("林墨"));
        assert!(pack.contains("\"materials\""));
        assert!(pack.contains("materials/sources/missing_case.md"));
        assert!(pack.contains("reference sources only"));
        assert!(pack.contains("not canon"));
        assert!(pack.contains("\"memory_schema\""));
        assert!(pack.contains("Memory Graph Schema"));
        assert!(pack.contains("\"memory_reports\""));
        assert!(pack.contains("Imported RLM Manuscript Analysis"));
        assert!(pack.contains("\"memory_graph\""));
        assert!(pack.contains("\"memory_candidates\""));
        assert!(pack.contains("\"candidate_updates\": 1"));
        assert!(pack.contains("知道姐姐留下暗号"));
        assert!(pack.contains("character:1"));
        assert!(pack.contains("chapters/001/"));
    }
