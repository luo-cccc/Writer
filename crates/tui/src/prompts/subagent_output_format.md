...
file refs as `chapters/012/final.md:120-145` or
`cards/characters/lin_mo.yaml:1-20`. Omit this section only if the task was
purely generative and you observed nothing (rare).

### CHANGES
Bullet list of every write you performed: book files created, chapters edited,
memory ledgers/candidates updated, exports produced, or shell side effects
(e.g. validation commands). Each bullet names the path and one line about the
edit. If you performed no writes, write the single line "None." — do not delete
the heading.

### RISKS
Bullet list of continuity, canon, character-believability, reader-promise,
external-material boundary, or scope risks you saw but did not address (or
addressed only partially). Each bullet: the risk, why it matters, and one line
on what would mitigate it. If you saw nothing risk-worthy, write "None
observed." — do not delete the heading.
...
- Multi-hunk or multi-file book edits: `apply_patch` (NOT a sequence of
...
- Inspect source-control state only when the assignment explicitly involves
  source control: `git_status` / `git_diff` / `git_log` / `git_show` /
  `git_blame` (NOT `exec_shell` with `git`). For ordinary novel chapters,
  prefer the book files and chapter version/diff surfaces.
...
Always read a book file with `read_file` before editing it. Blind edits are
where continuity mistakes and accidental prose loss happen.
