...
//!
//! The active Novel Studio path is `/audit <chapter>` for continuity and
//! memory diagnosis. `/review` is retained only as a skill compatibility hook.
...
pub fn review(app: &mut App, args: Option<&str>) -> CommandResult {
    ...
    if target.is_empty() {
        return CommandResult::error(
            "Usage: /audit <chapter>\n\nLegacy skill compatibility remains available as /review <target>, but novel work should use /audit for memory and continuity diagnosis.",
        );
    }
    ...
    let skill = match skill {
        ...
        None => {
            ...
            return CommandResult::error(format!(
                "Legacy review skill not found in {} or {}. Prefer `/audit <chapter>` for Novel Studio continuity diagnosis, or create ~/.deepseek/skills/review/SKILL.md for compatibility.{}",
                skills_dir.display(),
                global_display,
                warnings
            ));
...
mod tests {
    ...
    fn create_review_skill_dir(tmpdir: &TempDir) {
        ...
        std::fs::write(
            ...
            "---\nname: review\ndescription: Legacy continuity diagnosis skill\n---\nDiagnose the chapter memory",
        )
        .unwrap();
    }
    ...
    fn test_review_without_target() {
        ...
        assert!(msg.contains("Usage: /audit <chapter>"));
        assert!(msg.contains("Legacy skill compatibility"));
    }
    ...
    fn test_review_without_skill_installed() {
        ...
        let result = review(&mut app, Some("chapters/001/draft.md"));
