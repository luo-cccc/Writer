...
pub fn list_skills(app: &mut App, arg: Option<&str>) -> CommandResult {
    ...
    if registry.is_empty() {
        let msg = format!(
            "No writing skills found.\n\n\
             Skills location: {}\n\n\
             To add a local Novel Studio craft skill, create directories with SKILL.md files:\n  \
             {}/scene-pressure/SKILL.md\n\n\
             Format:\n  \
             ---\n  \
             name: scene-pressure\n  \
             description: Guide scene pressure without changing canon\n  \
             allowed-tools: read_file, list_dir, grep_files\n  \
             ---\n\n  \
             Read book.toml, bible/, outline/, cards/, chapters/, and memory/ as needed. Keep suggestions within canon and preserve user prose unless asked to rewrite.{warnings}",
            skills_dir.display(),
            skills_dir.display()
        );
    ...
    if filtered.is_empty() {
        ...
        return CommandResult::message(format!(
            "No writing skills match prefix `{p}` (out of {} available).\n\nRun /skills to see all craft packs.{warnings}",
            registry.len()
        ));
    }
    ...
    let mut output = if let Some(p) = prefix.as_deref() {
        format!(
            "Available writing skills matching `{p}` ({} of {}):\n",
            filtered.len(),
            registry.len()
        )
    } else {
        format!(
            "Available Novel Studio writing skills ({}):\n",
            registry.len()
        )
    };
    output.push_str("─────────────────────────────\n");
    ...
    let _ = write!(
        output,
        "\nUse /skill <name> to activate a craft pack\nSkills location: {}{}",
        skills_dir.display(),
        warnings
    );
...
fn activate_skill(app: &mut App, name: &str) -> CommandResult {
    ...
    if let Some(skill) = registry.get(name) {
        ...
        CommandResult::message(format!(
            "Writing skill '{}' activated.\n\nDescription: {}\n\nType your request and the craft guidance will be applied.",
            skill.name, skill.description
        ))
    } else {
        ...
        if available.is_empty() {
            CommandResult::error(format!(
                "Writing skill '{name}' not found. No craft packs installed.\n\nUse /skills to see how to add writing skills.{warnings}"
            ))
        } else {
            CommandResult::error(format!(
...
mod tests {
    ...
    struct IsolatedHome {
        ...
        appdata_prev: Option<OsString>,
        localappdata_prev: Option<OsString>,
        xdg_config_home_prev: Option<OsString>,
        xdg_data_home_prev: Option<OsString>,
        global_skill_dirs_prev: Option<Vec<PathBuf>>,
    }
    ...
    impl IsolatedHome {
        fn new(tmpdir: &TempDir) -> Self {
            ...
            let appdata = tmpdir.path().join("appdata");
            let localappdata = tmpdir.path().join("localappdata");
            let xdg_config = tmpdir.path().join("xdg_config");
            let xdg_data = tmpdir.path().join("xdg_data");
            ...
            let appdata_prev = std::env::var_os("APPDATA");
            let localappdata_prev = std::env::var_os("LOCALAPPDATA");
            let xdg_config_home_prev = std::env::var_os("XDG_CONFIG_HOME");
            let xdg_data_home_prev = std::env::var_os("XDG_DATA_HOME");
            let global_skill_dirs_prev = crate::skills::set_test_global_skill_dirs(Some(vec![
            ...
            Self {
                ...
                appdata_prev,
                localappdata_prev,
                xdg_config_home_prev,
                xdg_data_home_prev,
                global_skill_dirs_prev,
            }
        }
    ...
    impl Drop for IsolatedHome {
        fn drop(&mut self) {
            ...
            unsafe {
                ...
                Self::restore_var("APPDATA", self.appdata_prev.take());
                Self::restore_var("LOCALAPPDATA", self.localappdata_prev.take());
                Self::restore_var("XDG_CONFIG_HOME", self.xdg_config_home_prev.take());
                Self::restore_var("XDG_DATA_HOME", self.xdg_data_home_prev.take());
                crate::skills::set_test_global_skill_dirs(self.global_skill_dirs_prev.take());
            }
        }
    }
    ...
    fn test_list_skills_empty_directory() {
        ...
        assert!(msg.contains("No writing skills found"));
        assert!(msg.contains("Skills location:"));
        assert!(msg.contains("scene-pressure"));
        assert!(msg.contains("Guide scene pressure without changing canon"));
        assert!(msg.contains("book.toml"));
        assert!(msg.contains("memory/"));
        assert!(!msg.contains("my-skill"));
        assert!(!msg.contains("What this skill does"));
    }
    ...
    fn test_list_skills_with_skills() {
        ...
        assert!(result.message.is_some());
        let msg = result.message.unwrap();
        assert!(msg.contains("Available Novel Studio writing skills"));
        assert!(msg.contains("activate a craft pack"));
    ...
    fn test_list_skills_filter_with_zero_matches_says_so() {
        ...
        assert!(msg.contains("No writing skills match prefix `nonexistent`"));
    ...
    fn test_run_skill_activates() {
        ...
        assert!(result.message.is_some());
        let msg = result.message.unwrap();
        assert!(msg.contains("Writing skill 'test-skill' activated"));
