...
你正在 Writer 中运行。无论任务上下文（代码、错误日志、文件名）\
...
Writer を実行しています。タスクコンテキスト（コード、エラーログ、\
...
Você está rodando dentro do Writer. Escreva tanto \
...
mod tests {
    ...
    fn system_prompt_prepends_locale_preamble_for_zh_hans() {
        ...
        let base_marker = "You are Writer";
    ...
    fn compose_prompt_includes_all_layers() {
        ...
        // Base layer
        assert!(prompt.contains("You are Writer"));
        // Personality layer
    ...
    fn compose_prompt_deterministic_order() {
        ...
        let base_pos = prompt.find("You are Writer").unwrap();
