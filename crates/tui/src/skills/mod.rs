...
#[cfg(test)]
static TEST_GLOBAL_SKILL_DIRS: std::sync::Mutex<Option<Vec<PathBuf>>> = std::sync::Mutex::new(None);
...
pub fn skills_directories(workspace: &Path) -> Vec<PathBuf> {
    ...
    #[cfg(test)]
    if let Some(global_dirs) = TEST_GLOBAL_SKILL_DIRS
        .lock()
        .unwrap_or_else(|err| err.into_inner())
        .clone()
    {
        candidates.extend(global_dirs);
        return existing_skill_dirs(candidates);
    }
    if let Some(global_agents) = agents_global_skills_dir() {
...
#[cfg(test)]
pub(crate) fn set_test_global_skill_dirs(dirs: Option<Vec<PathBuf>>) -> Option<Vec<PathBuf>> {
    let mut guard = TEST_GLOBAL_SKILL_DIRS
        .lock()
        .unwrap_or_else(|err| err.into_inner());
    std::mem::replace(&mut *guard, dirs)
}
